...
class ParentProcess:
    ...
    def __init__(self, args, Server, Client):
        ...
        self.server = Server(args, self.sd, self.gpus) 
        atexit.register(self.done)

    def create_workers(self, Client):
        self.processes = []
        self.q = {}
        self.sd = multiprocessing.Manager().dict()
    ...
    def start(self):
        ...
        for curr_rnd in range(self.args.n_rnds):
            ...
            print(f'[server] all clients updated at round {curr_rnd}')
            ###########################################
            self.server.on_round_complete(self.updated)
            ###########################################
            print(f'[server] round {curr_rnd} done ({time.time()-st:.2f} s)')
    ...
    def done(self):
        for p in self.processes:
            p.join()
        print('all children joined, main process destroyed')
